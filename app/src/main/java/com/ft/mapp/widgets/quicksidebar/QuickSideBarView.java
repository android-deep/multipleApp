...
public class QuickSideBarView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mTextColorChoose = context.getResources().getColor(R.color.holo_yellow_dark);
...
//        mItemStartY = 0;
    ...
    public void setChooseLetter(int newChoose) {
//        Log.i("-----------choose",String.valueOf(newChoose));
        if (mChoose != newChoose) {
            if (newChoose >= 0 && newChoose < mLetters.size()) {
                mChoose = newChoose;
            }else{
                mChoose = mLetters.size()-1;
            }
            invalidate();
        }
    }
                ...
                if (oldChoose != newChoose) {
                    if (newChoose >= 0 && newChoose < mLetters.size()) {
                        ...
                        if (listener != null) {
                            ...
                            float yPos = mItemHeight * (mChoose-1) +
                                    (int) ((mItemHeight - rect.height()) * 0.5) + mItemStartY;
