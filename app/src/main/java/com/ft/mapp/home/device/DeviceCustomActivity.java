...
public class DeviceCustomActivity extends VActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ivBack = findViewById(R.id.mock_device_custom_iv_back);
...
//        setValue(edt_androidId, mDeviceConfig.androidId, "123456789");
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.device_save_tv) {
            ...
            edt_androidId.setText(randomCode(-1,16));
            edt_imei.setText(randomCode(-1,16));
            edt_imsi.setText(randomCode(-1,16));
            StringBuilder sbMac = new StringBuilder(randomCode(-1,2));
            for (int i = 0; i < 5; i++) {
                sbMac.append(":");
                sbMac.append(randomCode(-1,2));
            }
            edt_mac.setText(sbMac.toString());
        }
    }

    Random random = new Random();
    private char[] codes = new char[]{'1','2','3','4','5','6','7','8','9','0','a','b','c','d',
            'e','f'};
    private String randomCode(int range, int count){
        StringBuilder result = new StringBuilder();
        if (range==-1){
            range = codes.length;
        }
        for (int i = 0; i < count; i++) {
            result.append(codes[random.nextInt(range)]);
        }
